...
namespace RawMat.ViewsMaterial.RegularCheck
{
    internal static class ExportExcell
    {
        ...
        private static readonly string[] LegacyStampProtectionPasswords = { "B13", "RawMatRegularStamp" };
        ...
        private static void ApplyAutoReportRowLayout(Excel.Worksheet sheet, int topRow)
        {
            ...
            for (int row = topRow + 4; row <= topRow + 8; row++)
            {
                SetRowHeight(sheet, row, 21.6);
            }

            SetRowHeight(sheet, topRow + 7, 25.92);
            SetRowHeight(sheet, topRow + 8, 25.92);
            SetRowHeight(sheet, topRow + 9, 20.4);

            for (int row = topRow + 10; row <= topRow + 25; row++)
            {
        ...
        private static void BuildReportPage(Excel.Worksheet sheet, QAdataProperty dataItem, DataTable data, System.Collections.Generic.List<string> sampleNos, int topRow, int page, string referenceText, string imagePath)
        {
            ...
            MergeLeft(sheet, topRow, 2, topRow + 3, 22, reportHeader);
            StyleFont(sheet, topRow, 2, topRow + 3, 22, 13, true);


            SetLabel(sheet, topRow, 23, topRow, 27, "Report No.");
            SetLabel(sheet, topRow, 28, topRow, 31, "Approve");
            Merge(sheet, topRow + 1, 23, topRow + 3, 27, dataItem?.Report_No);
            Merge(sheet, topRow + 1, 28, topRow + 3, 31, string.Empty);

            SetLabel(sheet, topRow + 4, 2, topRow + 4, 6, "Vender");
            MergeLeft(sheet, topRow + 4, 7, topRow + 4, 31, dataItem?.Vendor_Name);
            SetLabel(sheet, topRow + 5, 2, topRow + 5, 6, "Receipt Date");
            Merge(sheet, topRow + 5, 7, topRow + 5, 17, FormatDate(dataItem?.dtReceiveDate));
            SetLabel(sheet, topRow + 5, 18, topRow + 5, 22, "INV. No.");
            Merge(sheet, topRow + 5, 23, topRow + 5, 31, dataItem?.Invoice_No);
            SetLabel(sheet, topRow + 6, 2, topRow + 6, 6, "Lot Size");
            Merge(sheet, topRow + 6, 7, topRow + 6, 17, dataItem?.Qty);
            SetLabel(sheet, topRow + 6, 18, topRow + 6, 22, "Lot No.");
            Merge(sheet, topRow + 6, 23, topRow + 6, 31, dataItem?.Lot_No);
            SetLabel(sheet, topRow + 7, 2, topRow + 7, 6, "Inspection Size");
            Merge(sheet, topRow + 7, 7, topRow + 7, 17, dataItem?.SAMPLING_QTY);
            SetLabel(sheet, topRow + 7, 18, topRow + 7, 22, "Reference");
            Merge(sheet, topRow + 7, 23, topRow + 7, 31, referenceText);
            SetLabel(sheet, topRow + 8, 2, topRow + 8, 6, "Inspection Date");
            Merge(sheet, topRow + 8, 7, topRow + 8, 17, DateTime.Now.ToString("dd-MMM-yyyy"));
            SetLabel(sheet, topRow + 8, 18, topRow + 8, 22, "Inspector");
            Merge(sheet, topRow + 8, 23, topRow + 8, 31, dataItem?.EMP_ID);
        ...
        private static void BuildAutoReportTable(Excel.Worksheet sheet, DataTable data, System.Collections.Generic.List<string> sampleNos, int tableTop)
        {
            ...
            for (int rowOffset = 0; rowOffset < AutoReportTableBodyRows; rowOffset++)
            {
                ...
                Merge(sheet, rowIndex, 5, rowIndex, 6, NumberDisplay.Trim(GetString(point, "CRITERIA_MIN")));
                Merge(sheet, rowIndex, 7, rowIndex, 8, NumberDisplay.Trim(GetString(point, "CRITERIA_MAX")));
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    Merge(sheet, rowIndex, col + 2, rowIndex, col + 3, NumberDisplay.Trim(GetString(sampleRow, "VALUE")));
        ...
        private static void BuildTable(Excel.Worksheet sheet, DataTable data, System.Collections.Generic.List<string> sampleNos, int tableTop, int startCol = 1)
        {
            ...
            foreach (var pointGroup in points)
            {
                ...
                sheet.Cells[rowIndex, startCol + 1] = NumberDisplay.Trim(GetString(point, "CRITERIA_MIN"));
                sheet.Cells[rowIndex, startCol + 2] = NumberDisplay.Trim(GetString(point, "CRITERIA_MAX"));
                ...
                for (int i = 0; i < sampleNos.Count; i++)
                {
                    ...
                    sheet.Cells[rowIndex, col + 1] = NumberDisplay.Trim(GetString(sampleRow, "VALUE"));
                    string judge = GetString(sampleRow, "POINT_JUDGE");
                    if (judge == "0")
                    {
                        totalJudge = "0";
        ...
        /// <summary>
        /// Reports are no longer protected after stamping, but files approved by earlier versions still
        /// are, so every password those versions used has to be tried before the sheet can be edited.
        /// </summary>
        private static void UnprotectStampSheet(Excel.Worksheet sheet)
        {
            ...
            foreach (string password in LegacyStampProtectionPasswords)
            {
                try
                {
            ...
            throw new InvalidOperationException(
                "ไม่สามารถปลดล็อก Sheet ของรายงานนี้ได้ (รหัสป้องกันไม่ตรงกับที่ระบบรู้จัก)");
        }
        ...
        private static void AddStampImage(Excel.Worksheet sheet, string stampImagePath)
        {
            ...
            try
            {
                ...
                if (!TryAddStampPictureInCell(approveCell, excelStampPath))
                {
                    throw new InvalidOperationException(
                        "ไม่สามารถวาง E-Stamp ลงในเซลล์ได้ (Place in Cell) " +
                        "กรุณาตรวจสอบว่าเครื่องนี้ใช้ Excel for Microsoft 365 ที่รองรับรูปภาพในเซลล์");
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Places the stamp as the value of the Approve cell instead of as a floating picture, so it
        /// cannot be dragged out of the block. Excel for Microsoft 365 only: the members are absent from
        /// the Office 2013 PIA this project references, so they are called late-bound and any failure
        /// falls back to the floating picture. Range.InsertPictureInCell is deliberately not used - it
        /// returns E_FAIL from automation on every Excel build tested, both early and late bound.
        /// </summary>
        private static bool TryAddStampPictureInCell(Excel.Range approveCell, string stampImagePath)
        {
            if (Thread.CurrentThread.GetApartmentState() != ApartmentState.STA)
            {
                // PastePictureInCell reads the clipboard, which is only available on an STA thread.
                return false;
            }
            ...
            try
            {
                targetCell = (Excel.Range)approveCell.Cells[1, 1];
                savedClipboard = CaptureClipboard();

                using (Image stampImage = Image.FromFile(stampImagePath))
                {
                    Clipboard.SetImage(stampImage);
                }

                if (HasInCellPicture(targetCell))
                {
                    targetCell.ClearContents();
                }

                targetCell.GetType().InvokeMember(
                    "PastePictureInCell",
                ...
                return HasInCellPicture(targetCell);
            }
            catch
            {
                return false;
        ...
        private static bool HasInCellPicture(Excel.Range cell)
        {
            try
            {
                object hasRichDataType = cell.GetType().InvokeMember(
                    "HasRichDataType",
                ...
                return Convert.ToBoolean(hasRichDataType);
            }
            catch
            {
                // Excel builds without the in-cell picture feature do not expose the property at all.
                return false;
            }
        }

        /// <summary>
        /// Copies the clipboard contents out format by format. The object returned by
        /// Clipboard.GetDataObject only wraps the live clipboard, so it goes stale as soon as the stamp
        /// image is copied over it and cannot be used to put the user's data back.
        /// </summary>
        private static DataObject CaptureClipboard()
        {
            try
            {
                System.Windows.Forms.IDataObject current = Clipboard.GetDataObject();
                if (current == null)
                {
                    return null;
                }
                ...
                foreach (string format in current.GetFormats())
                {
                    ...
                    {
                        // Some formats refuse to marshal; keep whatever else was captured.
                    }
                }
        ...
        private static void RestoreClipboard(DataObject savedClipboard)
        {
            try
            {
            ...
            {
                // Best effort only; losing the previous clipboard contents must not fail an approval.
            }
        }
        ...
        private static bool TryBuildStampRangeBelowLabel(Excel.Worksheet sheet, Excel.Range labelCell, out Excel.Range stampRange)
        {
            ...
            try
            {
                ...
                int stampEndRow = stampStartRow + 1; // default fallback
                Excel.Range nextCell = null;
                Excel.Range nextMergeArea = null;
                try
                {
                    nextCell = (Excel.Range)sheet.Cells[stampStartRow, column];
                    if (Convert.ToBoolean(nextCell.MergeCells))
                    {
                        nextMergeArea = nextCell.MergeArea;
                        int nextRowCount = Convert.ToInt32(nextMergeArea.Rows.Count);
                        stampEndRow = stampStartRow + nextRowCount - 1;
                    }
                }
                catch
                {
