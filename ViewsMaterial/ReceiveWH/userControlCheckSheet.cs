    ...
    public partial class userControlCheckSheet : UserControl
    {
        ...
        private async Task RefreshReceiveDataAsync(string refreshType)
        {
            ...
            try
            {
                ...
                await Task.Run(() => ProcessData(receiveDate, showMessages));

                // บันทึกทุกครั้งที่ค้นหา ไม่ว่าจะเจอข้อมูลหรือไม่
                // ป้ายบอกว่า Last update คือ "ค้นหาล่าสุดเมื่อไหร่" ไม่ใช่ "เจอของล่าสุดเมื่อไหร่"
                // ถ้าบันทึกเฉพาะตอนเจอ วันที่ไม่มีของจะดูเหมือนระบบไม่ได้ทำงาน
                // แยกไม่ออกกับกรณี auto refresh ค้างจริงๆ
        ...
        {
            // ออกใบเฉพาะตอนกดที่ช่องรูปรถเข็น (STATUS) เท่านั้น
            // เดิมคลิกช่องไหนของแถวก็ออกใบทันที เผลอคลิกดูข้อมูลก็ออกใบไปแล้ว
            if (e.ColumnIndex < 0 || dtg_receiveMat.Columns[e.ColumnIndex].Name != "STATUS")
            {
                return;
            }
            ...
            if (e.RowIndex >= 0 && e.RowIndex < dtg_receiveMat.Rows.Count)
            {
                ...
                // ยืนยันก่อนออกใบ เพราะกดแล้วเขียน DB และลบแถวออกจากตารางทันที กดผิดย้อนไม่ได้
                using (var frm = new frmConfirm(
                    "M-CODE : " + qaProp.M_CODE + Environment.NewLine +
                    "Invoice : " + qaProp.Invoice_No,
                    imgCls.LoadAppImage("cart1.png"),
                    "Issue check sheet"))
                {
