...
namespace RawMat.ViewsMaterial.Main
{
    public partial class userControlSearch : UserControl
    {
                ...
                if (columnName == "Regular No")
                {
                        ...
                        || regularCheck == ((int)QAdataProperty.ProcStatus.OK).ToString())
                    {
                        ...
                        try
                        {
                            ...
                            bool waitFileExists = System.IO.File.Exists(waitFilePath);
                            bool approvedFileExists = System.IO.File.Exists(approvedFilePath);

                            if (approvedFileExists)
                            {
                                OpenApprovedRegularReport(approvedFilePath);
                                return;
                            }

                            if (!waitFileExists)
                            {
                                MessageBox.Show($"Regular Report file was not found for {propQA.Regular_No}.\n\nPaths checked:\n- {waitFilePath}\n- {approvedFilePath}",
                                    "Regular report file not found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show("Cannot check Regular Report file path: " + ex.Message,
        ...
        private void OpenApprovedRegularReport(string approvedFilePath)
        {
            try
            {
                Process.Start(new ProcessStartInfo
                {
                    FileName = approvedFilePath,
                    UseShellExecute = true
                });
            }
            catch (Exception ex)
            {
                MessageBox.Show("Cannot open approved Regular Report file: " + ex.Message,
