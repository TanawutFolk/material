            ...
            if (e.RowIndex >= 0 && e.RowIndex < dtg_dimensionPending.Rows.Count && e.ColumnIndex >= 0 && e.ColumnIndex < dtg_dimensionPending.Columns.Count)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else if (propQA.SAMPLING_TYPE == "1")
                        {
                            // แบบ All เคยตกหล่นตรงนี้ ใบที่ Pending จึงเปิดไม่ได้เลย
                            // จำนวนมาจาก Lot Size ตรงๆ เหมือนหน้ากรอกค่า
                            string lotSizeText = dtg_dimensionPending.Rows[e.RowIndex].Cells["Lot Size"].Value?.ToString();

                            if (int.TryParse(lotSizeText, out int lotSize) && lotSize > 0)
                            {
                                propQA.SAMPLING_QTY = lotSize.ToString();
                            }
                        }
                        else
                        {
